...
public class ShoppingCartController {
    ...
    void onCalculate(ActionEvent event) {
        ...
        for (int i = 0; i < numItems; i++) {
            ...
            nameDialog.setHeaderText(bundle.getString("enter.name") + " #" + (i + 1));
            String name = nameDialog.showAndWait().orElse("Item " + (i + 1));
            ...
            try {
                if (itemDAO != null) {
                    itemDAO.addItem(price, quantity, currentLocale.getLanguage(), name);
                }
            } catch (Exception e) {
                new Alert(Alert.AlertType.ERROR, "Error saving item to database: " + e.getMessage()).show();
            }

            Label itemLabel = new Label(name + " — " + bundle.getString("total.cost") + ": " + itemTotal);
